...
export default async function handler(req, res) {
  ...
  if (!data) {
    data = { id: userId, username: tgUsername, points: 0, coins: 0, messagesSent: 0, level: 1, avatar: '👤', bio: '', chatId: '', ownedAvatars: '[]', ownedItems: '[]' };
  ...
  // Always update level and leaderboard on login
  await kv.hset(key, { level: String(level) });
  ...
  res.json({
    ...
    ownedItems: JSON.parse(data.ownedItems || '[]'),
  });
}
